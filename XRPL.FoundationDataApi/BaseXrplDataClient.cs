...
public abstract class BaseXrplDataClient
{
    ...
    /// <returns></returns>
    protected async Task<BaseServerResponse<TEntity>> GetAsync<TEntity>(string url, CancellationToken Cancel = default) where TEntity : class, new()
    {
        Remaining -= 1;
        ...
        {
            return new BaseServerResponse<TEntity>()
            {
                Response = response
            };
        }
        ...
        if (response.StatusCode == HttpStatusCode.BadRequest)
        {
            try
            {
                var error_data = await response.Content.ReadAsStringAsync();
                var error = string.IsNullOrWhiteSpace(error_data) ? new ApiErrorInfo() : JsonConvert.DeserializeObject<ApiErrorInfo>(error_data, serializerSettings);
                return new BaseServerResponse<TEntity>()
                {
                    Response = response,
                    ErrorInfo = error
    ...
    protected BaseXrplDataClient(bool waitWhenLimit, string apiKey, string BaseServiceAddress = "https://data.xrplf.org/")
    {
